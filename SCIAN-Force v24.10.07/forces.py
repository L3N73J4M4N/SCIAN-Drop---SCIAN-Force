...
from utility_forces import str2num, anti_nan, \
    ...
    get_angles, mixed_area, ldl_solve, get_point_area
...
# propiedades.
class Forces3D:
    def __init__(self, path=None, mesh=None, interval='1x1x1', units='px'):
    ...
    def plot_go(self, curvatures=True, edges=False):
        vertices, faces = self.mesh
        x, y, z = vertices[:, 0], vertices[:, 1], vertices[:, 2]
        i, j, k = faces[:, 0], faces[:, 1], faces[:, 2]
        if curvatures is True:
            color = cmap_go(self.meanCurvature)
            title_colorbar = 'Mean<br>Curvatures' if self.stress_bool is False else 'Normal<br>Stress<br>[nN/μm²]'
            title = self.name + ' - ' + self.method
            fig = go.Figure(layout={'title': title})
            fig.add_mesh3d(x=x, y=y, z=z, i=i, j=j, k=k,
    ...
    def optimized_laplacian(self):
        ...
        # del método de las cotangentes, o su expresión discreta.
        for i in range(0, len(vertices)):
            area = mixed_area(i, vertices, faces, triangles, angles)
            sum_laplacian = 0
            for j in points[i]:
                alph, bet = angles_triangles(i, j, neighbors, angles, faces)
                sum_laplacian += (cot(alph) + cot(bet)) * (vertices[j] - vertices[i])
    ...
    def rusinkiewicz_curvature(self, force_normal=True, ldl_numpy=True):
        ...
        # áreas
        point_areas, corner_areas = get_point_area(vertices, faces)
